...
void printPatterns(std::vector<std::vector<std::tuple<std::string, int, int>>> patternsFound, std::vector<std::string> patterns) { // O(n*p) -> n de tamaño de patrones a encontrar, p de patrones encontrados
  for (int i = 0; i < patternsFound.size(); i++) {
    if (patternsFound[i].empty()) {
...
bool zAlgoMultiplePatterns(std::vector<std::string> patterns, std::string text) { // O(n*p) -> n del tamaño del string, p del numero de patrones a comparar
  int lineIndex = 0;
  int fileLine = 1;
  ...
  std::vector<std::vector<std::tuple<std::string, int, int>>> patternsFound(
      patterns
          .size()); // (pattern, lineInFileOfPattern, indexWherePatternWasFound)
  zArr[L] = 0;
...
std::vector<std::string> readFileToVector(std::string filename) { // O(n) -> del numero de lineas del archivo
...
readFileToVectorAndString(std::string filename) { // O(n) -> del numero de lineas del archivo
  ...
  std::tuple<std::string, std::vector<std::string>> fileResult =
      std::make_tuple(resultingString, resultingVector);
  return fileResult;
}

void searchPatternInFile(std::string stringTransmission, std::string msg) { // O(n*p)
  std::vector<std::string> patternCode1 = readFileToVector("mcode1.txt"); // TODO: MAL RUTA DE ARCHIVO -> no va a correr en Clion
  std::vector<std::string> patternCode2 = readFileToVector("mcode2.txt");
  std::vector<std::string> patternCode3 = readFileToVector("mcode3.txt");
...
std::tuple<std::string, int, int> getPalindrome(std::string text, int start, int end) { // O(n) -> del texto que reciba
...
std::string makeStringOdd(std::string &text) { // O(n) -> del texto que reciba
  std::string result;
  for (char i : text) {
    result += '#';
    result += i;
...
std::tuple<std::string, int, int> manacher(std::string text) { // O(n) -> del string que reciba como parametro
...
void largestPalindrome(std::vector<std::string> texts, std::string msg) { // O(n*m) -> numero de lineas del archivo * tamaño de string a revisar
  ...
  for (auto text : texts) { // O(n)
    lineInFile++;
    std::tuple<std::string, int, int> palindromeFound = manacher(text); // O(m) -> m siendo el tamaño del string a revisar
    if (std::get<0>(palindromeFound).length() >
        std::get<0>(bestPalindrome).length()) {
      bestPalindrome = palindromeFound;
      palindromeFile = lineInFile;
    }
  }
  ...
  std::cout << msg << "\n";
  std::cout << "\t Palindromo mas largo encontrado: "
            << std::get<0>(bestPalindrome) << "\n";
  std::cout << "\t Linea: " << palindromeFile << "  -   indice inicial:" << std::get<1>(bestPalindrome) << "  -   indice final: " << std::get<2>(bestPalindrome) << "\n";
}

int main() {
  std::tuple<std::string, std::vector<std::string>> transmission1 = readFileToVectorAndString("transmision1.txt");
  std::tuple<std::string, std::vector<std::string>> transmission2 = readFileToVectorAndString("transmision2.txt");
  std::cout << "\nParte 1\n";
  // Parte 1 - casi casi falta indice correcto
  searchPatternInFile(std::get<0>(transmission1), "Transmición 1");
  searchPatternInFile(std::get<0>(transmission2), "Transmición 2");
